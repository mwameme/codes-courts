...
Chemin::Chemin(Coordonnee const& a,Coordonnee const& b)
{
    ...
    this->m_chemin.push_back(a);//le chemin commence par a
    this->m_chemin.push_back(b);//puis b
...
CarteLabyrinthe::CarteLabyrinthe(std::string const& m_adresse)
{
    ...
    for (int i(0); i<m_listeChemins2.size(); ++i)
    {
        ...
        {
            if (this->m_listeChemins[j] == m_listeChemins2[i])
            {
                trouve = true;
                break;
            }
        ...
        {
            this->m_listeChemins.push_back(m_listeChemins2[i]);
        }
    }
    ...
    this->m_carte =m_carte; //bon on sauvegarde les changements ... j'aurai du utiliser des this-> partout, erreur d'écriture ...
    this->m_listePlaces=m_listePlaces;
    this->m_listeChemins=m_listeChemins;
    this->m_Matrice=m_Matrice;
    this->m_solution=m_solution;
    this->m_nbPlaces=nbPlaces;

    cout<<nbPlaces<<endl;

    cout<<this->m_solution<<endl; //imprimer la solution ! et c'est fini !
}
